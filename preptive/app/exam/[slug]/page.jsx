...
import { GraduationCap, BarChart3, Ticket, BookCheck, Newspaper, Calendar, FileText, Award, Users } from 'lucide-react';
...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    const boardName = exam.exam_board?.name || '';
...
export default async function ExamPage({ params }) {
  ...
  try {
    // Fetch exam details with board information
    ...
    // Fetch posts for this exam with proper joins
    const { data: posts, error: postsError } = await supabase
      ...
      .select(`
        ...
        post_exam_map!inner (
          exam:examinations!inner (id, name, slug)
        )
      `)
      .eq('status', 'published')
      .eq('post_exam_map.exam.slug', slug)
    ...
    // Get latest posts by category for quick access
    const latestByCategory = uniqueCategories.map(category => {
      const categoryPost = posts?.find(post => 
        post.post_category_map?.some(pc => pc.category?.id === category.id)
      );
      return {
        category,
        post: categoryPost
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              recognizedBy: {
                '@type': 'Organization',
                name: exam.exam_board?.name || 'Government of India'
              },
              about: {
                '@type': 'ItemList',
        ...
        <main className="px-4 py-1 mx-auto max-w-7xl sm:px-6 lg:px-8">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    {exam.exam_board && (
                      <p className="text-gray-600">Conducted by: {exam.exam_board.name}</p>
                    )}
                  </div>
                </div>
          ...
          <div className="mt-16 p-6">
            <div className="prose prose-lg max-w-none text-gray-700">
              ...
              {exam.exam_board && (
                <p>
                  <strong>{exam.name}</strong> is conducted by <strong>{exam.exam_board.name}</strong>, 
              ...
              <ul className="space-y-2">
                <li>• <strong>Conducting Body:</strong> {exam.exam_board?.name || 'Various Government Agencies'}</li>
                <li>• <strong>Exam Level:</strong> National/State Level</li>
                <li>• <strong>Frequency:</strong> Annual/Bi-annual</li>
                <li>• <strong>Eligibility:</strong> Varies based on educational qualification and age limit</li>
                <li>• <strong>Selection Process:</strong> Multiple stages including preliminary, mains, and interviews</li>
              </ul>
