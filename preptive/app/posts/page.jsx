...
export default async function PostsPage({ searchParams }) {
  ...
  const [postsResult, categoriesResult, examsResult, countResult] = await Promise.allSettled([
    ...
    supabase
      ...
      .select(`
        ...
        exams:post_exam_map(examinations(id, name, exam_boards(name)))
      `)
      .eq('status', 'published')
  ...
  const totalPosts = countResult.status === 'fulfilled' ? countResult.value.count : 0;
  
  const totalPages = Math.ceil(totalPosts / postsPerPage);
  
  // ========== SEO ENHANCEMENTS ==========
  
  // Schema.org for Article List
        ...
        <div className="flex flex-wrap items-center justify-between mt-6 relative z-20">
          ...
          <footer className="mt-16 pt-8 border-t border-gray-200">
            <div className="prose prose-lg max-w-none text-gray-600">
              ...
              <p>
               PrepTive was created with one clear purpose — to help students stay informed and never miss an important exam update. In a fast-moving exam environment, even a small delay in information can cost an entire year. PrepTive exists to make sure that doesn't happen.

We focus on delivering latest exam notifications, admit card releases, result updates, important exam dates, syllabus revisions, exam pattern changes, and recruitment news in a clear and reliable manner. Our platform covers all major competitive and government exams in India, including UPSC, SSC, Banking, Railways, JEE, NEET, CLAT, state-level exams, scholarships, and university entrance tests.
              </p>
              <p>
               Every update published on PrepTive is verified from official sources and explained in simple, easy-to-understand language. Instead of just posting headlines, we break down what the update actually means for students — eligibility details, next steps, deadlines, and key points that truly matter.

PrepTive is built for serious aspirants who want accurate information, timely alerts, and zero confusion. Whether you are tracking a government job notification, waiting for an admit card, or checking the latest exam schedule, PrepTive helps you stay one step ahead.

Clear information. Timely updates. One trusted platform for every exam.
              </p>
